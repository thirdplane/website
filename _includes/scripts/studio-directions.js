...
(() => {
  ...
  const notes = {
    ...
    research:{title:'Steering by recognition: can an image model learn a quality we struggle to describe?',meta:'Independent research pilot / June 2026 / One week / $25 compute',question:'Can a recognizable visual quality become a dependable control inside an image model?',built:'A pilot that compared reference images, changed internal model activity, and reviewed shuffled output sequences with the settings hidden.',learned:'The images changed, but the pilot did not establish reliable control of the intended quality. AI feature scores only partly reproduced the author’s grouping, which also drifted on repeat sorting.',next:'How do we check whether an AI evaluation system is measuring the quality people actually care about?',path:'model-internals-research'}
  };
  let opener;
