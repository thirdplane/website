---
name: motion-dev-springs
description: Generate physics-based CSS spring and bounce animations using the linear() easing function via the Motion Dev MCP tools (generate-css-spring, generate-css-bounce-easing). Use when creating hover states, entrance animations, staggered reveals, or drop-in effects on this site that need natural, non-linear motion instead of hand-picked easing curves.
---
...
Check `_includes/styles/tokens.css` for the site's current `--spring-*` custom properties before generating new ones — the values there are the live source of truth, not this file.
